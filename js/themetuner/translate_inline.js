    ...
    formShow: function() {
        ...
        var t = new Template(
            '<div class="magento_table_container"><table cellspacing="0">' +
                ...
                '</td></tr>' +*/
                '<tr><th class="label"><label for="custom_#{i}">Translation:</label></th><td class="value">' +
                    '<input name="translate[#{i}][original]" type="hidden" value="#{scope}::#{original_escape}"/>' +
                    '<input name="translate[#{i}][preset_id]" type="hidden" value="#{preset_id}"/>' +
        ...
        for (i = 0; i < data.length; i++) {
            ...
            data[i]['preset_id'] = this.escapeHTML(data[i]['preset_id']);
