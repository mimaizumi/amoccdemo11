	...
	$.extend($.fn, {
		...
		miniColors: function(o, data) {
			...
			var show = function(input) {
				...
				//used colors hack
				var usedHolder = generateUsedColors();
				if( usedHolder.length ){
					var newWidth = selector.width() + usedHolder.outerWidth() + 4;
					selector.css({ 'width': newWidth });
					selector.append(usedHolder);
					
					$('#usedColors div', selector).click(function (event) {
						var color = convertColors( $(this).css('background-color') );
						input.val( color );
				...
				var fromtop = jQuery(window).innerHeight() - input.data('trigger').offset().top - selector.outerHeight() - 10;
				if( fromtop < 0 ){
					selector.css({
						top: input.is(':visible') ? input.offset().top - selector.outerHeight() : input.data('trigger').offset().top - selector.outerHeight(),
					})
				}
				selector.fadeIn(100);
